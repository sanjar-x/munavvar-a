...
## [Unreleased] — Walk-in warehouse-pickup: автоматическая тара-возврат по составу корзины (fix)

Источник: бизнес-уточнение от заказчика. Walk-in покрывает три сценария:

| Состав `items` | Что физически делает клиент | Что должен записать ledger |
|---|---|---|
| **A.** только вода (`[water]`) | Приносит пустую тару, обменивает на полную бутыль | `WAREHOUSE_SALE` (вода+тара) + `WAREHOUSE_TARA_RETURN` (тара) |
| **B.** вода + тара (`[water, container]`) | Покупает полную бутыль и доп. пустую тару | `WAREHOUSE_SALE` (вода+2 тары); ничего не сдаёт |
| **C.** только тара (`[container]`) | Покупает пустую тару отдельно | `WAREHOUSE_SALE` (тара); ничего не сдаёт |

Предыдущая ревизия (`082b92b`) для walk-in выкинула `WAREHOUSE_TARA_RETURN`
полностью — это покрыло сценарий B/C, но сломало сценарий A: клиент
физически сдавал тару, а ledger записывал её как ушедшую со склада.

### Changed

- `BaseOrderService._handle_warehouse_pickup`: для walk-in возврат тары
  выводится автоматически из payload — возвращается только
  `returnable_item_id`, который НЕ присутствует в `order.items` как
  самостоятельная позиция. Никаких новых полей API не добавлено,
  правила выводятся из состава корзины.
- В `WAREHOUSE_SALE` для walk-in `sale_items` теперь не дублирует тару,
  если она уже есть в `items` как явная покупка (предотвращение двойного
  списания со склада).

### Frontend impact

- API контракт **не изменился**. Фронт продолжает слать тот же payload.
- В сценарии A (только вода) `total_amount` = цена воды (тара
  обменивается). В сценарии B (вода+тара) — цена воды + цена тары.
  В сценарии C — только цена тары. Бэкенд складывает по `unit_price`
  каждого item (как и раньше).
- В ответе `stock_transfers` для сценария A снова появится
  `WAREHOUSE_TARA_RETURN` (исправляет mismatch с физикой склада).
