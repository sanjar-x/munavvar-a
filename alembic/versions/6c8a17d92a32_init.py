...
ENUM_DEFINITIONS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("product_type_enum", ("water", "container", "equipment")),
    (
        "user_role_enum",
        (
            "system",
    ...
    ("account_type_enum", ("revenue", "cash", "card", "bank", "discount", "client", "courier")),
    ("auth_provider_enum", ("local", "google", "telegram", "apple")),
    ("inventory_type_enum", ("WAREHOUSE", "COURIER", "CLIENT", "VIRTUAL_LOSS", "VIRTUAL_VENDOR")),
    ("payment_method_enum", ("cash", "card", "contract")),
    (
        "order_status_enum",
    ...
    ("transfer_status_enum", ("DRAFT", "COMPLETED", "CANCELLED")),
    ("transaction_status_enum", ("pending", "completed", "rejected")),
)


def _pg_enum(name: str, *values: str, create_type: bool = False) -> postgresql.ENUM:
...
def upgrade() -> None:
    ...
    for enum_name, enum_values in ENUM_DEFINITIONS:
        _pg_enum(enum_name, *enum_values, create_type=True).create(bind, checkfirst=True)
    ...
    sa.Column('type', _pg_enum('product_type_enum', 'water', 'container', 'equipment'), nullable=False, comment='Категория продукта (WATER, EQUIPMENT, CONTAINER)'),
    ...
    sa.Column('payment_method', _pg_enum('payment_method_enum', 'cash', 'card', 'contract'), nullable=False, comment='Намерение клиента по оплате: Наличные, Карта и т.д.'),
    sa.Column('status', _pg_enum('order_status_enum', 'new', 'assigned', 'in_transit', 'arrived', 'delivered', 'pickup_completed', 'cancelled'), nullable=False, comment='Текущий статус жизненного цикла заказа'),
    ...
    sa.Column('type', _pg_enum('transfer_type_enum', 'COURIER_LOAD', 'COURIER_RETURN', 'CLIENT_DELIVERY', 'CLIENT_RETURN', 'LOSS_WRITE_OFF', 'INVENTORY_FINDING', 'INITIAL_BALANCE', 'WAREHOUSE_SALE', 'WAREHOUSE_TARA_RETURN'), nullable=False),
    sa.Column('status', _pg_enum('transfer_status_enum', 'DRAFT', 'COMPLETED', 'CANCELLED'), nullable=False, comment='Статус документа (DRAFT, COMPLETED, CANCELLED)'),
    sa.Column('id', sa.UUID(), nullable=False, comment='ID (UUIDv7)'),
    sa.Column('is_active', sa.BOOLEAN(), server_default=sa.text('true'), nullable=False, comment='Флаг активности записи. False означает логическое удаление.'),
    sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False, comment='Дата и время создания записи'),
    sa.Column('updated_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False, comment='Дата и время последнего обновления'),
    ...
    sa.Column('status', _pg_enum('transaction_status_enum', 'pending', 'completed', 'rejected'), nullable=False, comment='Статус (PENDING - ожидание, COMPLETED - исполнено, REJECTED - отмена)'),
...
def downgrade() -> None:
    ...
    bind = op.get_bind()
    for enum_name, enum_values in reversed(ENUM_DEFINITIONS):
        _pg_enum(enum_name, *enum_values, create_type=True).drop(bind, checkfirst=True)
    # ### end Alembic commands ###
