...
const getName = (id) =>
  warehouses?.find((w) => w.id === id)?.name ?? virtualInventoryName(id); // см. раздел 5
```
...
_Spec подготовлен на основе анализа исходного кода бэкенда и фронтенда по состоянию на 2026-03-27._
